...
Aggregations were categorized “successful” if they contained four or more individuals as this was stated within existing literature to contribute to reproductive success. Crevice depth, width, and competitive purple sea urchin (Strongylocentrotus purpuratus) density was measured to assess their influence on abalone aggregation sizes. 
...
urchin <- read_csv("data/clean_transects.csv")
```
...
ggplot(ab_aggs, aes(x = fct_infreq(site), y = prop, fill = presence)) +
